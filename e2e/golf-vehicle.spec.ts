...
test('Carro: cabine, quatro suspensoes, direcao e montagem aberta', async ({ page }, testInfo) => {
  ...
  await expect.poll(async () => (await inspect()).filter(object => object.visible).length).toBe(10);
  ...
  await expect.poll(async () => (await inspect()).every(object => object.visible)).toBe(true);
